...
mysql -h $DB_HOST -u $DB_ADMIN --password=$DB_ADMIN_PASSWORD $DATABASE < "$SQL_ROOT/mysql.ddl"
echo "sample data"
mysql -h $DB_HOST -u $DB_ADMIN --password=$DB_ADMIN_PASSWORD $DATABASE < "$SQL_ROOT/mysql-data.sql"
